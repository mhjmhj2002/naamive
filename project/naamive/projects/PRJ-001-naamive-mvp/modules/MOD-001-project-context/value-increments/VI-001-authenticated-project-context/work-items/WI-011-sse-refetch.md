# WI-011 — SSE Invalidation and Canonical Refetch
...
Refresh browser context through SSE invalidation hints followed by canonical refetch.

## Out of scope

Durable queue semantics in SSE/NOTIFY; canonical entity payloads as SSE truth.

## Dependency satisfaction condition

WI-009 and WI-010 = DONE; Activity Center/query invalidation contract stable.
...
- GET /api/events
- one stream per AppShell/tab
- hint only
- reconnect jitter/backoff
- reconnect invalidates/refetches
- lost notification causes no permanent stale state

## Required tests

- one authenticated stream per AppShell/tab
- SSE contains hint/invalidation only
- reconnect backoff/jitter boundaries
- reconnect triggers query invalidation/refetch
- duplicate invalidations do not duplicate canonical mutation
- lost NOTIFY/SSE still converges through polling/refetch/reconnect

## Required evidence

- SSE integration tests
- reconnect/lost-notification regression output
- frontend query invalidation tests
