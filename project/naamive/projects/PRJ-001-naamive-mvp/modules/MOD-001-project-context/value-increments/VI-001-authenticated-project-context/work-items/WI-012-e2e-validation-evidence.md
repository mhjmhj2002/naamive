# WI-012 — End-to-end Validation Evidence
...
**Depends on:** WI-003..WI-011, WI-013  
...
Consolidate end-to-end evidence for VI-001 Validation without replacing local WI tests.

## Out of scope

Fixing implementation defects inside this WI; declaring VI ACCEPTED; declaring Delivery.

## Dependency satisfaction condition

All required implementation WIs are DONE on one stable candidate baseline; no Execution in flight can alter it.
...
- denied Project E2E
- session-expired E2E
- restart/revocation proof
- SSE reconnect/refetch regression
- real PostgreSQL gates
- architecture guardrails
- factual Validation evidence attached; no auto-Delivery

## Required tests

- Playwright happy path login→authorized Project→context→Activity Center
- denied Project path
- session-expired path
- restart/revocation journey
- SSE reconnect/refetch journey
- real PostgreSQL integration/architecture gates included in evidence package

## Required evidence

- Playwright reports/traces/screenshots where applicable
- referenced per-WI test outputs
- identified integrated baseline and manifest
