# WI-004 — Durable Server-side Session
...
Create durable opaque server-side sessions with expiry, rotation and restart-safe revocation.

## Out of scope

Project authorization decisions; Project list; AppShell behavior.

## Dependency satisfaction condition

WI-003 = DONE; authenticated principal can be established without a durable session yet.
...
- HMAC/hash storage per TIR
- HttpOnly/SameSite/Secure policy
- absolute/idle timeout and rotation
- logout revocation
- restart does not erase valid or revive revoked session

## Required tests

- raw reusable session token is never stored in DB
- HMAC/hash lookup works and does not expose token
- cookie flags satisfy local/remote rules
- absolute and idle timeout boundaries
- rotation and previous-token overlap <= TIR limit
- logout revokes session
- valid/revoked sessions preserve behavior across web restart

## Required evidence

- database inspection proving no raw token
- session integration tests
- restart/revocation test outputs
