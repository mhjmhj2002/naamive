# WI-013 — Canonical Project Read Source
...
Materialize the minimum canonical read-only Project source and public query contract required by VI-001.

## Out of scope

Project lifecycle mutation commands; Project transition UI; Activity Center derived rows.

## Dependency satisfaction condition

WI-001 = DONE; Project technical boundary/schema available under frozen TB/TIR.
...
- canonical Project current/history source exists
- deterministic DEV/test fixture creates canonical facts only
- restart-safe currentness
- public query contract available
- no private cross-module persistence access
- no Project lifecycle mutation introduced

## Required tests

- clean migration creates Project current/history structures in real PostgreSQL
- deterministic DEV/test canonical Project seed survives restart
- current/history relationship and currentness constraints hold
- public Project query contract reads canonical source without private cross-module repository access
- no projection/board/log is accepted as Project canonical truth

## Required evidence

- migration/constraint integration outputs
- seed/restart evidence
- public query contract tests
