...
além do código, testes, schema/migrations e contratos afetados.
...
Respeitar integralmente a limitação de execução agregada documentada em
`/AGENTS.md`.

O agente deve executar:

- `npm run build`;
- testes focados de autenticação/RBAC;
- teste novo desta regressão;
- testes focados de projeção/UI afetados;
- demais regressões focadas diretamente relacionadas que sejam seguras;
- `git diff --check`;
- revisão de `git status`;
- revisão do `git diff`.

O agente **não deve executar por conta própria** `npm test`, `npm run e2e` ou
`macro-lifecycle.e2e.test.ts` enquanto a limitação descrita em `AGENTS.md`
permanecer vigente.

Quando essas validações agregadas forem necessárias, reportar:

```text
MANUAL_OPERATOR_VALIDATION_REQUIRED
```

sem classificá-las como PASS ou FAIL.

## Validações manuais do operador

Após a implementação e os testes focados, o operador executará, quando
necessário:
...
e fornecerá o resultado para certificação da correção.
...
Após implementar e passar os testes focados:
...
- build e suítes focadas obrigatórias;
- `git diff --check`;
- resultado das validações manuais do operador quando aplicáveis;
