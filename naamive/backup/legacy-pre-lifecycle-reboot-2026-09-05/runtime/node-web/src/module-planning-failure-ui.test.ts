...
test('F5-23 planning failure uses the authoritative projection rather than a stale phase-specific controller', () => {
  assert.match(page, /function renderProjection\(projection\)/);
  assert.match(page, /function renderActivity\(projection\)/);
  assert.match(page, /function renderStopSurfaces\(projection\)/);
  assert.match(page, /function renderActions\(projection\)/);
  assert.match(page, /projection\.next_action/);
  assert.doesNotMatch(page, /\?phase3=true/);
  assert.doesNotMatch(page, /f523Render|f523OpenProject|\/retry-plan/);
});
