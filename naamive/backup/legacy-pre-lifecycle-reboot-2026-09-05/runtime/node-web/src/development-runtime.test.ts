...
import { developmentReservationGraceSeconds, developmentRuntimeSanitize } from './development-runtime.js';
...
test('development runtime bounds the reservation grace to a positive numeric interval',()=>{
  assert.equal(developmentReservationGraceSeconds(300),300);
  assert.equal(developmentReservationGraceSeconds(0),1);
  assert.equal(developmentReservationGraceSeconds('not-a-number'),1);
});
