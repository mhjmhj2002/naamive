...
import { createProductCommitmentRevision } from './product-commitment.js';
...
export const persistDiscoveryAgentOutcome = async (client: pg.PoolClient, job: { id: string; kind: string; project_id: string; operation_id: string; revision_id: string | null }, result: AgentResult, executionArtifactHash?: string, createdBy?:string) => {
  ...
  const workflow=(await client.query(`SELECT workflow_code,workflow_version FROM projects WHERE id=$1`,[job.project_id])).rows[0];
  if(job.kind==='REVIEW_PRODUCT_COMMITMENT'&&result.result==='READY_FOR_GATE'&&workflow?.workflow_code==='PROJECT_DISCOVERY'&&Number(workflow.workflow_version)===4){
    if(!job.revision_id) throw new Error('PRODUCT_COMMITMENT_INTAKE_REVISION_MISSING');
    const requirements=(await client.query(`SELECT a.id,a.sha256 FROM artifacts a JOIN jobs j ON j.id=a.execution_id
      WHERE a.project_id=$1 AND a.artifact_type='product-requirements' AND j.revision_id=$2
      ORDER BY a.created_at DESC LIMIT 1`,[job.project_id,job.revision_id])).rows[0];
    if(!requirements) throw new Error('PRODUCT_COMMITMENT_REQUIREMENTS_ARTIFACT_MISSING');
    const revision=await createProductCommitmentRevision(client,job.project_id,result.evidence.product_commitment,{
      source_intake_revision_id:job.revision_id,
      source_requirements_artifact_id:requirements.id,
      source_review_artifact_id:artifact.id
    },`discovery-product-commitment:${job.operation_id}:${job.id}`,createdBy);
    const target=await transitionTarget(client,job.project_id,'PRODUCT_COMMITMENT_READY');
    await client.query('UPDATE projects SET state=$2,updated_at=clock_timestamp() WHERE id=$1',[job.project_id,target]);
    await event(client,job.project_id,'PRODUCT_COMMITMENT_READY',job.operation_id,job.id,job.revision_id,{agent:job.kind,phase:type,evidence_type:type,evidence_hash:artifact.hash,execution_artifact_hash:executionArtifactHash??null,result:result.result,product_commitment_revision_id:revision.id,canonical_sha256:revision.canonical_sha256,gate_record_id:revision.gate_record_id,next_action:'Aguardando decisão de compromisso de produto'});
    return;
  }
  let trigger = '', next = '';
