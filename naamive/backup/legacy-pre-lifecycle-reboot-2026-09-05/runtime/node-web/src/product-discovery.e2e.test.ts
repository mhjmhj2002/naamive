...
if (!process.env.DATABASE_URL) test('product discovery acceptance requires DATABASE_URL', { skip: 'set DATABASE_URL' }, () => {});
  ...
  const { pool }=await import('./db.js'); const { applyReviewAdjustments,retryProductDiscovery,startProductDiscovery,projectDetail }=await import('./service.js'); const { runOnce }=await import('./worker.js'); const { testAuthenticatedHeaders }=await import('./test-auth.js');
  ...
  test('controlled discovery waits for an operator adjustment before restarting requirements',async t=>{
    t.after(cleanup);
    const session=await testAuthenticatedHeaders(projectId,[{role_code:'OPERATOR',action_code:'OPERATE_PROJECT'}]);t.after(session.cleanup);
    ...
    for(let i=0;i<60;i++) { await runOnce(projectId); detail=await projectDetail(projectId); jobs=(await pool.query(`SELECT kind,status,last_error FROM jobs WHERE project_id=$1 ORDER BY available_at`,[projectId])).rows; if(detail.state==='WAITING_FOR_PRODUCT_COMMITMENT') break; if(jobs.some(job=>job.status==='FAILED')) break; }
    ...
    const readiness=await fetch(`http://127.0.0.1:${address.port}/api/agent/readiness`,{method:'POST',headers:session.headers}); assert.equal(readiness.status,200); assert.equal((await readiness.json()).ready,true);
    const response=await fetch(`http://127.0.0.1:${address.port}/api/projects/${projectId}/decision`,{method:'POST',headers:{...session.headers,'content-type':'application/json'},body:JSON.stringify({decision:'REJECTED',gate_id:detail.gate.id,version:detail.gate.version,feedback:'Detalhar restrições'})}); server.close(); assert.equal(response.status,200);
    detail=await projectDetail(projectId); assert.equal(detail.state,'REQUIREMENTS_IN_PROGRESS');
    process.env.NAAMIVE_CONTROLLED_REVIEW_RESULT='REQUIRES_ADJUSTMENT'; await runOnce(projectId); await runOnce(projectId); delete process.env.NAAMIVE_CONTROLLED_REVIEW_RESULT;
  ...
  test('registration keeps intake workflow until discovery starts and rejects invalid workflow states',async t=>{
    ...
    const session=await testAuthenticatedHeaders(id,[{role_code:'OPERATOR',action_code:'OPERATE_PROJECT'}]);t.after(session.cleanup);
  ...
  test('projects a configuration failure and retries discovery from its failed stage',async t=>{
    ...
    process.env.NAAMIVE_AGENT_ADAPTER='codex'; process.env.NAAMIVE_CODEX_WORKDIR=`/tmp/naamive-missing-${randomUUID()}`; for(let attempt=0;attempt<3;attempt++){await runOnce(id);await pool.query(`UPDATE jobs SET available_at=now() WHERE id=$1 AND status='RETRYABLE'`,[failedJob]);}
