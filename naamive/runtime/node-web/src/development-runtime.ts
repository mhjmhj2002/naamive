...
export const developmentRuntime=async(projectId:string,workItemId:string)=>{
 ...
 const history=(await pool.query(`SELECT d.id delivery_id,j.id job_id,j.status job_status,j.completed_at FROM deliveries d JOIN jobs j ON j.delivery_id=d.id WHERE d.work_item_id=$1 AND j.kind='DEVELOP_WORK_ITEM' AND j.status IN ('COMPLETED','FAILED') AND j.id<>$2 ORDER BY j.completed_at DESC,j.id DESC LIMIT 3`,[workItemId,selected.job_id??'00000000-0000-0000-0000-000000000000'])).rows.map((x:any)=>({...x,completed_at:iso(x.completed_at)}));
 let rule=leases.length>=2?'MULTIPLE_LEASED_JOBS':!selected.job_id?'DELIVERY_JOB_MISSING':!selected.worktree_id?'DELIVERY_WORKTREE_MISSING':null;
 ...
 const inc=rule?inconsistency(workItemId,selected,rule):null;const result:any={schema_version:VERSION,work_item_id:workItemId,attempt,inconsistency:inc,history,diagnostic_id:null};if(inc){const known=(await pool.query(`SELECT id FROM runtime_diagnostics WHERE work_item_id=$1 AND rule_code=$2 ORDER BY created_at DESC LIMIT 1`,[workItemId,inc.rule_code])).rows[0];result.diagnostic_id=known?.id??randomUUID();}try{developmentRuntimeSanitize(result);}catch{result.attempt=null;result.inconsistency=inconsistency(workItemId,null,'SANITIZATION_REJECTED');result.diagnostic_id=randomUUID();}return result;
};
export const detectDevelopmentRuntimeInconsistencies=async()=>{const items=(await pool.query(`SELECT DISTINCT d.project_id,d.work_item_id FROM deliveries d`)).rows;for(const x of items){const r=await developmentRuntime(x.project_id,x.work_item_id);if(!r.inconsistency)continue;const i=r.inconsistency,a=r.attempt;const fingerprint=createHash('sha256').update(`${VERSION}|${x.work_item_id}|${i.rule_code}|${i.delivery_id??'null'}|${i.job_id??'null'}|${a?.delivery_state??'null'}|${a?.worktree_state??'null'}|${a?.work_item_state??'null'}|${a?.job_status??'null'}`).digest('hex'),id=randomUUID();await withTransaction(async c=>{const ins=await c.query(`INSERT INTO runtime_diagnostics(id,work_item_id,fingerprint,rule_code,state_version) VALUES($1,$2,$3,$4,$5) ON CONFLICT DO NOTHING RETURNING id`,[id,x.work_item_id,fingerprint,i.rule_code,VERSION]);if(ins.rowCount)await c.query(`INSERT INTO events(project_id,event_type,correlation_id,payload,actor_id) VALUES($1,'DEVELOPMENT_RUNTIME_INCONSISTENT',$2,$3,$4)`,[x.project_id,randomUUID(),{work_item_id:x.work_item_id,diagnostic_id:id,fingerprint,rule_code:i.rule_code},'runtime-detector']);});}};
