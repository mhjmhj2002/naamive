...
export const freezeWorkItemDeliveryCandidate=async(client:PoolClient,input:{deliveryId:string;headSha:string;sourceOperationId:string;correlationId:string;sourceEventId?:number|null})=>{
  const row=(await client.query(`SELECT d.*,w.module_id,w.round_id,w.payload,w.workflow_code,w.workflow_version,w.technology_baseline_revision_id,
      m.current_revision_id,t.branch,j.id AS authoritative_job_id,p.repository_path,
      pr.id AS plan_revision_id,q.id AS qa_matrix_id,q.payload AS qa_matrix_payload,q.hash AS persisted_qa_matrix_hash
    ...
    LEFT JOIN module_plan_revisions pr ON pr.id=(w.payload->>'plan_revision_id')::uuid
  ...
  if(!row.plan_revision_id)return null;
  ...
  const workItemRevisionId=deterministicHash({contract:'work-item-revision:v1',module_plan_revision_id:row.plan_revision_id,work_item_id:row.work_item_id,payload:row.payload});
...
const executeReviewHandoff=async(intent:any)=>withTransaction(async client=>{
  ...
  const assurancePolicy=(await client.query(`SELECT * FROM assurance_policies WHERE enabled=true AND (selectors->'agentPolicyNames' IS NULL OR selectors->'agentPolicyNames' ? $1) AND (selectors->'taskTypes' IS NULL OR selectors->'taskTypes' ? 'DEVELOP_WORK_ITEM') AND (selectors->'classifications' IS NULL OR selectors->'classifications' ? 'INTERNAL') ORDER BY published_at DESC LIMIT 1`,[executionPolicy?.name??''])).rows[0];
  if(!executionPolicy||!assurancePolicy){
  ...
  const acceptance=await createAcceptance(client,{id:execution.id,project_key:candidate.project_id,policy_name:execution.policy_name,task_type:'DEVELOP_WORK_ITEM',classification:'INTERNAL',agent_id:execution.agent_id,agent_version:execution.agent_version,selected_runtime_id:execution.selected_runtime_id,selected_configuration_version:execution.selected_configuration_version,policy_id:execution.policy_id,policy_version:execution.policy_version},candidate.correlation_id);
  if(!acceptance)throw new ApiError(409,'AUT02_ASSURANCE_POLICY_SELECTION_CHANGED');
