...
if(!process.env.DATABASE_URL){
  ...
  test('AUT-02 runs frozen QA, dispatches independent review, and only Assurance ACCEPT enables merge',async t=>{
    ...
    await pool.query(`INSERT INTO module_plan_revisions(id,project_id,module_id,revision_number,module_revision_id,payload,payload_hash,json_artifact_hash,markdown_artifact_hash,author_id,status,work_item_workflow_code,work_item_workflow_version,integration_pipeline_version) VALUES($1,$2,$3,1,$4,$5,$6,$6,$6,'test','APPROVED','WORK_ITEM_DELIVERY',2,'AUTOMATIC_ASSURANCE_INTEGRATION_PIPELINE:v1')`,[plan,project,module,moduleRevision,{work_items:[{work_item_id:'qa-1'}]},'a'.repeat(64)]);
    await pool.query(`INSERT INTO work_items(id,project_id,module_id,revision_id,round_id,title,payload,state,workflow_code,workflow_version,module_plan_revision_id,plan_work_item_id,integration_pipeline_version) VALUES($1,$2,$3,$4,$5,'QA WI',$6,'QA_IN_PROGRESS','WORK_ITEM_DELIVERY',2,$7,'qa-1','AUTOMATIC_ASSURANCE_INTEGRATION_PIPELINE:v1')`,[workItem,project,module,moduleRevision,round,{work_item_id:'qa-1',plan_revision_id:plan,acceptance_criteria:['QA and independent ACCEPT'],qa_matrix:[{command:'test -f result.txt',cwd:'.',timeout_seconds:10}],depends_on_ids:[]},plan]);
    ...
    const frozen=await withTransaction(client=>freezeWorkItemDeliveryCandidate(client,{deliveryId:delivery,headSha:head,sourceOperationId:operation,correlationId:correlation}));assert.ok(frozen?.id);assert.equal(frozen.plan_work_item_id,'qa-1');
