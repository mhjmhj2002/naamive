...
if(!process.env.DATABASE_URL){
  ...
  test('AUT-02 runs frozen QA, dispatches independent review, and only Assurance ACCEPT enables merge',async t=>{
    ...
    await pool.query(`INSERT INTO work_items(id,project_id,module_id,revision_id,round_id,title,payload,state,workflow_code,workflow_version,module_plan_revision_id,plan_work_item_id) VALUES($1,$2,$3,$4,$5,'QA WI',$6,'QA_IN_PROGRESS','WORK_ITEM_DELIVERY',2,$7,'qa-1')`,[workItem,project,module,moduleRevision,round,{work_item_id:'qa-1',plan_revision_id:plan,acceptance_criteria:['QA and independent ACCEPT'],qa_matrix:[{command:'test -f result.txt',cwd:'.',timeout_seconds:10}],depends_on_ids:[]},plan]);
