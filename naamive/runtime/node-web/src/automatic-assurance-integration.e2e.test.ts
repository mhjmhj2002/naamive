...
if(!process.env.DATABASE_URL){
  ...
  const {AUT02_V1_PIPELINE_VERSION,AUT02_V1_POLICY_VERSION,enqueueAut02Intent}=await import('./aut02-ledger.js');
  ...
  test('AUT-02 creates one immutable multi-WI candidate only after the last merge and finalizes all members atomically',async t=>{
    ...
    await pool.query(`INSERT INTO module_plan_revisions(id,project_id,module_id,revision_number,module_revision_id,payload,payload_hash,json_artifact_hash,markdown_artifact_hash,author_id,status,work_item_workflow_code,work_item_workflow_version,integration_pipeline_version) VALUES($1,$2,$3,1,$4,$5,$6,$7,$8,'test','APPROVED','WORK_ITEM_DELIVERY',2,$9)`,[plan,project,module,moduleRevision,{work_items:logical.map(item=>({work_item_id:item.work_item_id}))},h('plan'),h('plan-json'),h('plan-md'),AUT02_V1_PIPELINE_VERSION]);
    ...
    for(let index=0;index<logical.length;index++){
      ...
      await pool.query(`INSERT INTO work_items(id,project_id,module_id,revision_id,round_id,title,payload,state,workflow_code,workflow_version,module_plan_revision_id,plan_work_item_id,integration_pipeline_version) VALUES($1,$2,$3,$4,$5,$6,$7,'ACCEPTED','WORK_ITEM_DELIVERY',2,$8,$9,$10)`,[workItem,project,module,moduleRevision,round,`WI ${index+1}`,{work_item_id:logical[index].work_item_id,plan_revision_id:plan,acceptance_criteria:['accepted'],qa_matrix:[{command:'true',cwd:'.',timeout_seconds:10}],depends_on_ids:[]},plan,logical[index].work_item_id,AUT02_V1_PIPELINE_VERSION]);
    ...
    await pool.query(`INSERT INTO work_items(id,project_id,module_id,revision_id,round_id,title,payload,state,workflow_code,workflow_version) VALUES
      ...
      ($3,$4,$5,$11,$7,'wrong revision',$12,'ACCEPTED','WORK_ITEM_DELIVERY',2)`,[decoys[0],decoys[1],decoys[2],project,module,moduleRevision,round,{work_item_id:'wi-1'},otherRound,{work_item_id:'wi-2'},otherRevision,{work_item_id:'wi-3'}]);
    ...
    assert.equal(Number((await pool.query(`SELECT count(*)::int n FROM integration_candidates WHERE project_id=$1 AND pipeline_version=$2`,[project,AUT02_V1_PIPELINE_VERSION])).rows[0].n),0);
    ...
    assert.equal((await pool.query(`SELECT plan_work_item_id FROM work_items WHERE id=$1`,[facts[2].workItem])).rows[0].plan_work_item_id,'wi-3','payload drift does not change normative identity');
    ...
    const candidates=(await pool.query(`SELECT * FROM integration_candidates WHERE project_id=$1 AND pipeline_version=$2`,[project,AUT02_V1_PIPELINE_VERSION])).rows;assert.equal(candidates.length,1);const candidate=candidates[0];
    ...
    assert.equal(Number((await pool.query(`SELECT count(*)::int n FROM integration_candidates WHERE project_id=$1 AND pipeline_version=$2`,[project,AUT02_V1_PIPELINE_VERSION])).rows[0].n),1);
