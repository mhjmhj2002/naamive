...
if(!process.env.DATABASE_URL){
  ...
  test('AUT-02 creates one immutable multi-WI candidate only after the last merge and finalizes all members atomically',async t=>{
    ...
    for(let index=0;index<logical.length;index++){
      ...
      await pool.query(`INSERT INTO work_items(id,project_id,module_id,revision_id,round_id,title,payload,state,workflow_code,workflow_version,module_plan_revision_id,plan_work_item_id) VALUES($1,$2,$3,$4,$5,$6,$7,'ACCEPTED','WORK_ITEM_DELIVERY',2,$8,$9)`,[workItem,project,module,moduleRevision,round,`WI ${index+1}`,{work_item_id:logical[index].work_item_id,plan_revision_id:plan,acceptance_criteria:['accepted'],qa_matrix:[{command:'true',cwd:'.',timeout_seconds:10}],depends_on_ids:[]},plan,logical[index].work_item_id]);
