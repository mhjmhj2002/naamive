...
import { authorizeRework, authorizeWorkItem, completeDefinition, createCandidate, decideArchitecture, decideModule, materializeModule, phase3Detail, startDevelopment, submitQa } from './phase3.js';
...
export const createApiServer = () => createServer(async (request, response) => { try {
  ...
  const phase3Match = url.pathname.match(/^\/api\/projects\/([^/]+)\/modules(?:\/([^/]+)(?:\/(decision|work-items|definition|architecture))?)?$/);
