...
const decide = async (projectId: string, body: Record<string, unknown>) => withTransaction(async (client) => {
  ...
  if(product){const operationId=randomUUID(),jobId=randomUUID(),approvedKind=approved?'PREPARE_TECHNOLOGY_SELECTION_CONTEXT':'PRODUCT_DISCOVERY';await client.query(`INSERT INTO operations(id,project_id,kind,status,idempotency_key,correlation_id,revision_id,workflow_code,workflow_version) VALUES($1,$2,$3,'QUEUED',$4,$5,$6,$7,$8)`,[operationId,projectId,approvedKind,approved?`selection-context:${row.id}:${row.version}`:`rework:${row.id}:${row.version}`,correlation,row.revision_id,approved?'PROJECT_DISCOVERY':null,approved?3:null]);await client.query(`INSERT INTO jobs(id,operation_id,project_id,revision_id,kind,idempotency_key) VALUES($1,$2,$3,$4,$5,$6)`,[jobId,operationId,projectId,row.revision_id,approved?'PREPARE_TECHNOLOGY_SELECTION_CONTEXT':'DEFINE_PRODUCT_REQUIREMENTS',approved?`selection-context:${operationId}`:`rework-requirements:${operationId}`]);}
