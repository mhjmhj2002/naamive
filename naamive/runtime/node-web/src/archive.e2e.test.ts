...
if (!process.env.DATABASE_URL) test('archive acceptance requires DATABASE_URL', { skip: 'set DATABASE_URL' }, () => {});
  ...
  const make=async(state:string,gate=false)=>{const projectId=`archive-e2e-${randomUUID().slice(0,8)}`,revisionId=randomUUID(),operationId=randomUUID(),jobId=randomUUID(); const draft={title:'Arquivar',business_owner:'Operações'};
    await pool.query(`INSERT INTO projects(id,title,business_owner,submitted_by,repository_path,repository_origin,base_branch,initial_sha,workflow_code,workflow_version,state,draft) VALUES($1,$2,$3,'test','/tmp','test://origin','main','000','PROJECT_DISCOVERY',1,$4,$5)`,[projectId,draft.title,draft.business_owner,state,draft]);
