...
export const startTechnologyInventory = async (client: pg.PoolClient, projectId: string, idempotencyKey: string) => {
  ...
  await putArtifact(client, projectId, 'technology-inventory', JSON.stringify({ schema_version: 1, status: 'RESERVED', requested_sha: project.initial_sha, selection_context_id: context.id, technology_catalog_revision_id: context.technology_catalog_revision_id }), jobId);
...
export const executeTechnologyInventory = async (client: pg.PoolClient, job: any) => {
  ...
  try {
    ...
    if (git(temp,['rev-parse',sha]) !== sha) throw new Error('INVENTORY_SNAPSHOT_SHA_DIVERGED');
    const items=(await client.query(`SELECT ri.catalog_item_id,ri.is_active,ci.code FROM technology_catalog_revision_items ri JOIN technology_catalog_items ci ON ci.id=ri.catalog_item_id WHERE ri.revision_id=$1`,[context.technology_catalog_revision_id])).rows;
    ...
    const snapshot={schema_version:1,requested_sha:sha,read_sha:sha,selection_context_id:context.id,technology_catalog_revision_id:context.technology_catalog_revision_id,facts:rows}; const hash=createHash('sha256').update(JSON.stringify(snapshot)).digest('hex'); await putArtifact(client,job.project_id,'technology-inventory',JSON.stringify({...snapshot,content_hash:hash}),job.id);
