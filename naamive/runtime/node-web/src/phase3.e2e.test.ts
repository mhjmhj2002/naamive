...
if (!databaseUrl) {
  ...
  const { createDevelopmentTelemetrySink } = await import('./development-telemetry.js');
  ...
  test('freezes QA, sanitizes timeout findings, revalidates and validates a frozen candidate', async (t) => {
    ...
    execFileSync('git',['-C',firstTree,'reset','--hard',git.sha]);
    await pool.query(`UPDATE operations SET status='FAILED' WHERE id=$1`,[started.operation_id]);
    await pool.query(`UPDATE jobs SET status='FAILED' WHERE operation_id=$1`,[started.operation_id]);
    ...
    await runOnce(id); assert.equal((await pool.query(`SELECT state FROM work_items WHERE id=$1`,[workItemId])).rows[0].state,'QA_IN_PROGRESS');
    assert.ok(Number((await pool.query(`SELECT count(*)::int AS count FROM artifacts WHERE project_id=$1 AND artifact_type='development-execution-evidence'`,[id])).rows[0].count)>=2);
    const retryEvent=(await pool.query(`SELECT event_type FROM events WHERE project_id=$1 AND event_type='DEVELOPMENT_RETRY_REQUESTED'`,[id])).rows[0]; assert.ok(retryEvent);
    const retryJob=(await pool.query(`SELECT * FROM jobs WHERE operation_id=$1`,[retry.operation_id])).rows[0]; await pool.query(`UPDATE jobs SET status='LEASED' WHERE id=$1`,[retryJob.id]); await createDevelopmentTelemetrySink(retryJob).heartbeat(); await pool.query(`UPDATE jobs SET status='COMPLETED' WHERE id=$1`,[retryJob.id]);
    assert.equal((await pool.query(`SELECT count(*)::int AS count FROM events WHERE job_id=$1 AND event_type='DEVELOPMENT_TELEMETRY_HEARTBEAT'`,[retryJob.id])).rows[0].count,1);
    execFileSync('mkdir',['-p','src'],{cwd:firstTree});execFileSync('sh',['-lc','printf first > src/delivery.txt'],{cwd:firstTree});execFileSync('git',['-C',firstTree,'add','src/delivery.txt']);execFileSync('git',['-C',firstTree,'-c','user.name=naamive-bot','-c','user.email=naamive-bot@localhost','commit','-m',`feat(${workItemId}): delivery\n\nNaamive-Project: ${id}\nNaamive-Phase: 3\nNaamive-Execution: first\nNaamive-Work-Item: ${workItemId}`]); await submitQa(id,workItemId,{},`qa-${randomUUID()}`); const rejected=(await pool.query(`SELECT validations FROM deliveries WHERE work_item_id=$1 ORDER BY created_at DESC LIMIT 1`,[workItemId])).rows[0].validations[0]; assert.equal(rejected.timed_out,false); assert.equal(rejected.severity,'HIGH'); assert.equal((await pool.query(`SELECT count(*),max(severity) severity FROM findings WHERE project_id=$1 AND origin='DELIVERY_QA'`,[id])).rows[0].count,'1'); assert.equal((await pool.query(`SELECT max(severity) severity FROM findings WHERE project_id=$1 AND origin='DELIVERY_QA'`,[id])).rows[0].severity,'HIGH');
