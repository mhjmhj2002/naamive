...
if (!process.env.DATABASE_URL) {
  ...
  test('serves the web journey, SSE replay, submission, worker and registration decision', async (t) => {
    ...
    const submitted = await fetch(`${base}/api/projects/${projectId}/submit`, { method: 'POST', headers: { 'idempotency-key': `http-${projectId}` } }); assert.equal(submitted.status, 202); assert.equal(await runOnce(projectId), true);
