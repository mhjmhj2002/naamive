...
| I-005 | F2-01 | `NON_BLOCKING` | `OPEN` | O operador precisará remover projetos de desenvolvimento/teste da lista principal sem perder rastreabilidade ou apagar acidentalmente projetos com operações, gates e artefatos. A Fase 1 não expõe arquivamento. | Implementar na Fase 2 uma operação administrativa explícita de `ARCHIVE_PROJECT`: exclusão lógica no PostgreSQL com `archived_at`, `archived_by`, `archive_reason` e `archived_from_state`; evento auditável; bloqueio/cancelamento governado de trabalho ativo; ocultação na lista padrão e filtro de arquivados. Gravar `archive/projects/<project-id>/archive-record.json` no ArtifactStore com ator, motivo, data e referências canônicas. Não mover artefatos existentes: URI, chave e hash permanecem imutáveis em `artifacts/`. Não alterar o repositório Git externo. Exclusão física permanece fora de escopo até decisão de retenção e backup. | `YES` |
