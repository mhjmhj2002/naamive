...
| I-005 | F2-01 | `NON_BLOCKING` | `OPEN` | O operador precisará remover projetos de desenvolvimento/teste sem perder rastreabilidade ou apagar acidentalmente projetos com operações, gates e artefatos. A Fase 1 não expõe exclusão. | Projetar e implementar na Fase 2 uma operação administrativa explícita de arquivamento/exclusão lógica, com confirmação, motivo, ator, evento auditável, bloqueio de operações ativas e política de retenção de artefatos. A exclusão física só poderá ser considerada após decisão de retenção e backup. | `YES` |
| I-006 | F1-07 / F1-13 | `BLOCKING` | `OPEN` | A criação de rascunho retornou `INTERNAL_ERROR` ao operador. A resposta sanitizada não oferece causa ou próxima ação, impedindo diferenciar falha de configuração, vínculo Git ou persistência. | Reproduzir com dados do ambiente local, registrar causa sanitizada no servidor com correlação e retornar código/ação segura e específica ao cliente; acrescentar teste de regressão. Nunca expor stack trace, caminhos não autorizados ou credenciais. | `PENDING` |
| I-007 | F1-07 | `NON_BLOCKING` | `OPEN` | O servidor local inicia sem informar endereço/porta, configurações efetivas seguras ou erros sanitizados de requisição no terminal. Isso impede diagnosticar `INTERNAL_ERROR` e torna a operação de desenvolvimento opaca. | Emitir logs estruturados e sanitizados de startup, requisição com falha e encerramento, incluindo método, rota, status, código de erro e correlação quando disponível. Nunca registrar payload de intake, credenciais, paths não autorizados ou stack trace no cliente. Critérios de aceite: `npm run dev` informa URL local; falhas retornam código ao navegador e log seguro ao terminal; logs não expõem dados sensíveis. | `PENDING` |
