...
| I-008 | F1-04 / F1-07 | `BLOCKING` | `OPEN` | O clone `/home/mhj/git/central-atendimento` foi validado manualmente: está na allowlist, possui Git, `origin`, commit, `origin/main` e árvore limpa. Ainda assim, a criação pela UI retorna `REPOSITORY_INVALID`. | Reproduzir a chamada da API com o clone validado, identificar a etapa exata do vínculo Git que falha e retornar código específico; adicionar teste de regressão para clone com `origin/main` válido. | `PENDING` |
