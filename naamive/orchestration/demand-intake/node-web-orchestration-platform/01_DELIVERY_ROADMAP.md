...
| I-022 | F3-09 | `BLOCKING` | `OPEN` | A validação PostgreSQL/E2E da Fase 3 exige `DATABASE_URL`, obtido pelo script somente via `.env`; a instrução desta execução proíbe acessar esse arquivo. Os unitários e o smoke Git local foram executados, mas o aceite web controlado permanece sem evidência factual. | Executar migrations, PostgreSQL/E2E e aceite web controlado em ambiente que injete `DATABASE_URL` sem expor o `.env`; anexar a evidência antes de concluir F3-09. | `NOT_REQUIRED` |
