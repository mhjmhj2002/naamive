...
| I-012 | F2-06 | `BLOCKING` | `OPEN` | A execução padrão de `npm run e2e` falha no aceite de descoberta: o projeto permanece em `ANALYSIS_IN_PROGRESS` quando o cenário espera `WAITING_FOR_PRODUCT_COMMITMENT`. Outros cenários concorrentes compartilham o lock global do worker, tornando o progresso do teste não determinístico. | Isolar os cenários E2E que usam worker global ou fazê-los aguardar a conclusão do próprio job/operação com polling correlacionado; repetir a suíte completa sem falhas e sem skips. | `PENDING` |
