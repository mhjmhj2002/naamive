...
| I-009 | F2-03 | `NON_BLOCKING` | `OPEN` | Smoke separado encontrou `codex-cli 0.144.0`, mas não recebeu confirmação de evidência estruturada do launcher dentro da janela de 30 s no ambiente local. O aceite controlado permanece válido. | Ajustar a instalação/credenciais do Codex e executar o smoke com um launcher que retorne o contrato JSON `result`/`evidence`; não persistir saída bruta. | `PENDING` |
| I-010 | F2-05 / F2-06 | `BLOCKING` | `OPEN` | O aceite controlado falha ao solicitar ajustes no gate `PRODUCT_COMMITMENT`: `POST /api/projects/<id>/decision` retorna `500 INTERNAL_ERROR`, com causa sanitizada `ENAMETOOLONG`. Os demais cenários E2E passam, mas o retorno governado a requisitos não foi validado. | Identificar o caminho/identificador excessivamente longo na persistência da decisão/artefato, aplicar chave imutável de tamanho seguro, adicionar regressão para solicitação de ajustes e repetir todo o aceite controlado. | `PENDING` |
| I-011 | F2-06 | `BLOCKING` | `OPEN` | `npm run e2e` não carrega `.env`; sem variáveis exportadas manualmente, todos os testes PostgreSQL são ignorados e o comando termina sem falha, produzindo falso sucesso de aceite. | Carregar `.env` nos scripts de teste/E2E ou falhar explicitamente quando `DATABASE_URL` e a configuração obrigatória estiverem ausentes; garantir que o E2E execute pelo menos um cenário de integração. | `PENDING` |
