...
| I-003 | F1-13 | `NON_BLOCKING` | `OPEN` | Ao selecionar um projeto, a UI recria a conexão SSE durante o tratamento de cada evento e volta a assinar a timeline desde o início. Isso causa renderização repetida, aparência de recarregamento/piscar e torna os eventos ilegíveis. Registros de aceite existentes também aparecem sem distinção visual de dados de demonstração. | Manter uma única assinatura SSE por projeto com cursor `after` monotônico; atualizar apenas projeções afetadas sem reabrir a timeline em cada evento; deduplicar eventos por ID e renderizá-los de forma estável. Melhorar a lista com data/estado e tratar dados de demonstração sem ocultar auditoria canônica. Critérios de aceite: selecionar projeto não recria stream repetidamente; cada evento aparece uma vez; tela não pisca; atualização de status/gate continua em tempo real; reconexão preserva cursor. | `PENDING` |
