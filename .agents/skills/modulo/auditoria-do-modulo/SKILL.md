...
Atue após receber a formação de um Módulo em `EM_FORMACAO`, sua Especificação Técnica, Direção de Projeto de origem e evidências pertinentes.
...
Compare a formação às fontes normativas e à Direção do Projeto. Diferencie insuficiência técnica de problema estrutural de delimitação. Quando houver insuficiência, descreva lacunas concretas e o responsável adequado sem resolver por conta própria.

## Saída e handoff

* `FORMACAO_INSUFICIENTE` técnico → **Especialista em Formação do Módulo**.
* `FORMACAO_INSUFICIENTE` por problema estrutural de delimitação → indicação ao **Especialista em Delimitação de Módulos**.
* `FORMACAO_SUFICIENTE` → Especificação Técnica do Módulo aprovada e disponível para consumo posterior; a continuação operacional não é definida.
...
Confirme independência do parecer, resultado formal único, separação entre Resultado e artefato, inexistência de status posterior inventado e preservação da fronteira com a vertical futura.
