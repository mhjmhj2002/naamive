...
O catálogo normativo de status do Projeto está em [Status do Projeto](STATUS_DO_PROJETO.md). O catálogo normativo de Resultados do Processo está em [Resultados do Processo do Projeto](RESULTADOS_DO_PROCESSO_DO_PROJETO.md). Conclusões de auditoria, decisões e verificações não devem ser tratados como status.
...
A pergunta conceitual dessa verificação é: **“O resultado agregado produzido atende ao compromisso que originou este Projeto?”** A conclusão positiva produz o Resultado do Processo `COMPROMISSO_ATENDIDO`; a conclusão negativa produz `COMPROMISSO_NAO_ATENDIDO`.
...
`CANCELAMENTO_APROVADO` é uma decisão humana material. O agente não pode produzi-la unilateralmente. A definição do responsável pelo Projeto, da matriz de permissões e das regras detalhadas de cancelamento em cada ponto do ciclo permanece para atividade posterior.
...
| `CANCELAMENTO_APROVADO` | posição não terminal → `CANCELADO` | Caminho excepcional; o agente não pode produzi-lo unilateralmente. |
