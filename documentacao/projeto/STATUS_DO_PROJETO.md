...
| `EM_MODULOS` | A formação do Projeto foi suficiente e os Módulos necessários foram materializados. O Projeto encerrou sua responsabilidade direta de formação e decomposição, e sua realização passa a ser conduzida pelos Módulos. Ele continua existindo como entidade pai, agregadora e referência do compromisso recebido da Necessidade. |
| `CONCLUIDO` | Status terminal de sucesso. O Projeto somente o assume quando o trabalho necessário de seus Módulos estiver concluído e a verificação agregada confirmar que o resultado produzido atende ao compromisso recebido da Necessidade. Representa o encerramento bem-sucedido do Projeto. |
| `CANCELADO` | Status terminal excepcional. O Projeto somente o assume por decisão humana válida de cancelamento. Um Projeto cancelado não representa compromisso atendido. |
...
* verificação agregada que confirme o atendimento ao compromisso recebido da Necessidade.
...
O Resultado do Processo ou a decisão formal que autoriza a transição para `CANCELADO` será definido separadamente. Este catálogo não define responsável, permissões ou mecanismo de cancelamento.
...
criação
→ EM_FORMACAO
→ formação suficiente + Módulos materializados
→ EM_MODULOS
→ verificação agregada suficiente
→ CONCLUIDO
```
...
O detalhamento das transições pertence ao [Ciclo de Vida do Projeto](CICLO_DE_VIDA_DO_PROJETO.md).
