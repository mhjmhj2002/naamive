...
`FORMACAO_SUFICIENTE` conclui e aprova o processo de formação e transiciona o Projeto de `EM_FORMACAO` para `FORMADO`. Ele torna disponível a Direção do Projeto, que é o artefato de saída consolidado dessa vertical, mas não se confunde com ela. `FORMADO` não produz realização, entrega, conclusão ou encerramento e não exige a definição, a decomposição, a materialização, o status ou o ciclo de vida de entidade descendente alguma.
